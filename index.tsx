...
const FLAGS: Record<string, string> = {
    ...
    'AR': 'data:image/png;base64,',
};
    ...
    const analyzeCreative = async (file: File) => {
        ...
        try {
            ...
            const response = await ai.models.generateContent({
                model: 'gemini-1.5-flash', // Usei o 1.5-flash, mas pode ser 'gemini-2.5-flash' se já tiver acesso
                contents: { parts: [imagePart, { text: prompt }] },
                generationConfig: {
                    responseMimeType: "application/json",
                },
                // @ts-ignore Ferramentas podem não estar no tipo padrão ainda
                tools: [{ functionDeclarations: [{ name: "analysisSchema", description: "Schema for analysis", parameters: analysisSchema }] }],
                toolConfig: { functionCallingConfig: { mode: "ANY" } }
            });
            
            // Acessando a resposta JSON da chamada de função
            const functionCall = response.candidates[0].content.parts[0].functionCall;
            if (functionCall && functionCall.name === 'analysisSchema') {
                const result = functionCall.args as AnalysisResult;
                setAnalysis(result);
            } else {
    ...
    const generateVariations = async () => {
        ...
        try {
            ...
            const textResponse = await ai.models.generateContent({
                model: 'gemini-1.5-pro', // Usar um modelo mais robusto para copywriting
                contents: { parts: [{ text: copyPrompt }] },
                generationConfig: {
                    responseMimeType: "application/json",
                },
                // @ts-ignore
                tools: [{ functionDeclarations: [{ name: "copyGenerationSchema", description: "Schema for copy variations", parameters: copyGenerationSchema }] }],
                toolConfig: { functionCallingConfig: { mode: "ANY" } }
            });
            
            const textFunctionCall = textResponse.candidates[0].content.parts[0].functionCall;
            if (!textFunctionCall || textFunctionCall.name !== 'copyGenerationSchema') {
                throw new Error("A IA não conseguiu gerar variações de texto.");
            }
            
            const result = textFunctionCall.args;
            ...
            const imageGenerationPromises = copyVariations.slice(0, 4).map((copy: any) => {
                ...
                return ai.models.generateContent({
                    model: 'gemini-1.5-flash', // ou o modelo de imagem que você tem acesso
                    contents: { parts: [{ text: imagePrompt }] },
                    // @ts-ignore
                    generationConfig: {
                        responseMimeType: "image/png", // Solicita a imagem diretamente
                    },
                });
            });
                    ...
                    try {
                        ...
                        const translationResponse = await ai.models.generateContent({
                            ...
                            generationConfig: {
                             ...
                             // @ts-ignore
                            tools: [{ functionDeclarations: [{ name: "generatedImageAnalysisSchema", description: "Schema for translation", parameters: generatedImageAnalysisSchema }] }],
                            toolConfig: { functionCallingConfig: { mode: "ANY" } }
                        });

                        const transFunctionCall = translationResponse.candidates[0].content.parts[0].functionCall;
                         if (!transFunctionCall || transFunctionCall.name !== 'generatedImageAnalysisSchema') {
                            throw new Error("A IA não conseguiu traduzir o texto da imagem.");
                        }

                        const result = transFunctionCall.args;
                        return {
                            src: imgSrc,
...
const VideoEditor = () => {
    ...
    const generateCountryElements = async () => {
        ...
        try {
            ...
            const response = await ai.models.generateContent({
                model: 'gemini-1.5-flash',
                contents: [{ parts: [{ text: `Para o país "${countryPrompt}", forneça informações para uma sobreposição de vídeo. Preciso do emoji da bandeira, o nome da moeda local, um valor monetário atraente para ganhar por uma pequena tarefa e um call-to-action.` }] }],
                generationConfig: {
                    responseMimeType: "application/json",
                },
                // @ts-ignore
                tools: [{ functionDeclarations: [{ name: "infoSchema", description: "Schema for country info", parameters: infoSchema }] }],
                toolConfig: { functionCallingConfig: { mode: "ANY" } }
            });
            
            const functionCall = response.candidates[0].content.parts[0].functionCall;
            if (!functionCall || functionCall.name !== 'infoSchema') {
                throw new Error("Resposta da IA não está no formato esperado.");
            }
            
            const info = functionCall.args;
    ...
    const generateCaption = async (prompt: string) => {
        ...
        try {
            const response = await ai.models.generateContent({
                model: 'gemini-1.5-flash',
                contents: [{ parts: [{ text: `Gere uma legenda curta e impactante para um vídeo com a seguinte descrição: "${prompt}". A legenda deve ter no máximo 15 palavras. Retorne apenas o texto da legenda.` }] }],
            });
            addElement('text', { content: response.response.text() });
...
// Este é o componente que controla qual ferramenta (Editor ou Gerador) está ativa

const App = () => {
  // 'image' para o Gerador de Imagens, 'video' para o Editor de Vídeo
    ...
    <div className="app-container">
      <nav className="app-nav">
        <button 
          ...
          className={currentView === 'image' ? 'active' : ''}
        >
          <ImageIcon /> Gerador de Criativos
        </button>
        <button 
          onClick={() => setCurrentView('video')}
          className={currentView === 'video' ? 'active' : ''}
        >
          <VideoIcon /> Editor de Vídeo
        </button>
      </nav>

      <main>
        {currentView === 'image' && <ImageCreator />}
        {currentView === 'video' && <VideoEditor />}
      </main>
    </div>
  );
};
...
// Esta é a parte que estava faltando e que faz o aplicativo aparecer na tela.
...
if (container) {
  ...
  console.error('Falha ao encontrar o elemento root. Verifique seu index.html');
}
